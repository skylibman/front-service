...
async function generateSign() {
  const signHeaders = {
    ...
    "X-Nonce": document.getElementById("nonce").value,
    "X-Timestamp": document.getElementById("timestamp").value,
  };
  ...
  const stringForSign = createURLSearchParamsString({
    ...signHeaders,
    ...payloadContents,
  });

  const xSign = CryptoJS.HmacSHA1(stringForSign, inputMerchKey.value).toString(
    CryptoJS.enc.Hex
  );

  const jsonSignCheck = JSON.stringify(
    { stringForSign: stringForSign, "X-Sign": xSign },
    null,
    2
  );
...
async function sendData() {
  ...
  const bodyData =
    contentType === "application/x-www-form-urlencoded"
      ? createURLSearchParamsString(payloadContents)
      : JSON.stringify(payloadContents);
  const stringForSign = createURLSearchParamsString({
    ...signHeaders,
    ...payloadContents,
  });
